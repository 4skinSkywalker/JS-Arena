...
function pushClient(roomId, client) {
    roomIdClientArray[roomId] = roomIdClientArray[roomId] || [];
    roomIdClientArray[roomId].push(client);
}

function getClients(roomId) {
    return roomIdClientArray[roomId] || [];
}
...
function removeRoomIfEmpty(roomId) {
    const clients = getClients(roomId);
    if (clients.length === 0) {
        log("Room", roomId, "is empty, deleting");
        delete roomIdClientArray[roomId];
    }
}
...
async function handleMessage(ws, msg) {
    ...
    switch (topic) {
        case "ping": {
            ...
            log("Ping received from", clientId);
            ws.sendMsg("pong");
            break;
        }
        case "roomIdResponse": {
            const roomId = message;
            log(ws.clientId, "responded to roomIdRequest with roomId", roomId);

            checkString({ roomId });

            const clients = getClients(roomId);
            if (clients && clients.includes(ws)) {
                log(ws.clientId, "already in room", roomId);
                return;
            }

            ws.roomId = roomId;
            pushClient(roomId, ws);

            log(ws.clientId, "is requested to introduce his/herself in room", roomId);
            ws.sendMsg("nameRequest", ws.clientId);
            break;
        }
        case "nameResponse": {
            ws.clientName = latinize(message);
            log(ws.clientId, "is named", ws.clientName);
        ...
        case "lastEditorContent": {
            ...
            log(ws.clientId, "sent last editor content");
        ...
        case "progress": {
            ...
            log(ws.clientId, "sent progress", percentage);
        ...
        case "chat": {
            log(ws.clientId, "sent chat message", message);
...
function handleClose(ws) {
    log(ws.clientId, "is leaving room", ws.roomId);

    const removed = removeClient(ws.roomId, ws.clientId);
    if (removed) {
        for (const client of getClients(ws.roomId)) {
...
function handleConnection(ws) {
    ...
    log("Client connected", ws.clientId);
